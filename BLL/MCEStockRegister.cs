...
namespace EuSoft.BLL
{
	...
	{
		private readonly EuSoft.DAL.MCEStockRegister dal=new EuSoft.DAL.MCEStockRegister();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public EuSoft.Model.MCEStockRegister GetModelByCache(int ID)
		{
			
			string CacheKey = "MCEStockRegisterModel-" + ID;
			...
			return (EuSoft.Model.MCEStockRegister)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<EuSoft.Model.MCEStockRegister> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<EuSoft.Model.MCEStockRegister> DataTableToList(DataTable dt)
		{
			List<EuSoft.Model.MCEStockRegister> modelList = new List<EuSoft.Model.MCEStockRegister>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				EuSoft.Model.MCEStockRegister model;
