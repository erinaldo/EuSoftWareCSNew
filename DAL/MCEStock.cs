...
namespace EuSoft.DAL
{
	...
	public partial class MCEStock
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "MCEStock"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from MCEStock");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(EuSoft.Model.MCEStock model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into MCEStock(");
			strSql.Append("StockCatalogNo,StockCSNo,StockSize,StockUnit,StockValCode,StockBatchNo,StockNote,StockLibraryID,SysNote,StockLocation,UpdateTime,Person)");
			...
			parameters[9].Value = model.StockLocation;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(EuSoft.Model.MCEStock model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update MCEStock set ");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from MCEStock ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public EuSoft.Model.MCEStock GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,StockCatalogNo,StockCSNo,StockSize,StockUnit,StockValCode,StockBatchNo,StockNote,StockLibraryID,SysNote,StockLocation,UpdateTime,Person from MCEStock ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
			...
			EuSoft.Model.MCEStock model=new EuSoft.Model.MCEStock();
		...
		/// 得到一个对象实体
		/// </summary>
		public EuSoft.Model.MCEStock DataRowToModel(DataRow row)
		{
			EuSoft.Model.MCEStock model=new EuSoft.Model.MCEStock();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append(@"select a.ID,StockCatalogNo,csno StockCSNo,StockSize,DrugNames,cas,mwt,StockUnit,StockValCode,StockBatchNo,StockNote,StockLibraryID,SysNote,StockLocation,UpdateTime,Person,rkrq ,replace(c.Storage,'4°C', '2-8°C') Storage ,cast( cast(cast(c.pruity as decimal(18,4))*100  as decimal(18,2)) as nvarchar(50))+'%'  pruity 
          FROM MCEStock a left join  MCEProductsBasicinfo b on a.StockCatalogNo=b.CatalogNo
          left join ProductsStorage c  on a.StockCatalogNo=c.CatalogNo ");

			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
            return DbHelperSQL.QueryMCE(strSql.ToString());
		}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from MCEStock T ");
